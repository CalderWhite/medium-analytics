...
export default class Donut extends Component{
  constructor(props){
    ...
    if(typeof(props.height) == 'string' && props.height.search('%') > -1){
        window.addEventListener('resize',()=>{
          let height = Number(props.height.replace(/\%/g,'')); 
          this.setHeightByPercent(height)
    ...
    this.state.height = Math.min(Math.max(this.state.height,props.minHeight),props.maxHeight);
    // bind methods
  ...
  setHeightByPercent(percent){
    // NOTE: it is .innerWidth, not innerHeight. I don't have time to impliment specifying which dimension it is relative to,
    // and for now I only want it to be relative to the width.
    let newHeight = window.innerWidth * percent / 100;
    this.setState({
        height : Math.min(Math.max(newHeight,this.props.minHeight),this.props.maxHeight)
    });
  }
  componentDidMount(){
    this.generateGraph();
