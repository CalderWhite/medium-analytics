...
var SERVER_BASE = "127.0.0.1:8080"; //"newspace-calderwhite.c9users.io";
var API_PATH = "/api";
var userInfo = {
    username: null,
    userId: null,
    ...
    if (url.toLowerCase().search("_/api/stream") > -1) {
        ...
        for (var i = 0; i < details.requestHeaders.length; i++) {
            if (details.requestHeaders[i].name.toLowerCase().search("cookie") > -1) {
                var cookieTemp = details.requestHeaders[i].value.split(";");
                for (var j = 0; j < cookieTemp.length; j++) {
                    var _cookieTemp$j$split = cookieTemp[j].split("="),
                        _cookieTemp$j$split2 = _slicedToArray(_cookieTemp$j$split, 2),
                        name = _cookieTemp$j$split2[0],
                        value = _cookieTemp$j$split2[1];
...
chrome.webRequest.onSendHeaders.addListener(checkSendHeaders, {
    urls: ['*://*.medium.com/*']
}, ['requestHeaders']);
